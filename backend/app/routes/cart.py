...
from ..services.cart_service import CartResponse, CartItemUpdate, CartItemCreate, CartService
...
def remove_from_cart(prouct_id: int, request: RemoveCartRequest, db: Session = Depends(get_db)):
    service = CartService(db)
    updated_cart = service.remove_from_cart(request.cart, prouct_id)
    return {"cart": updated_cart}